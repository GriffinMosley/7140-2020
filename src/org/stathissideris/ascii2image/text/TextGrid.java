...
public class TextGrid {
		...
		for(int yi = 0; yi < height; yi++){
			for(int xi = 0; xi < width; xi++){
				...
				if(StringUtils.isOneOf(c, pointMarkers)
					...
					if(StringUtils.isOneOf(get(cell.getEast().x, cell.getEast().y), horizontalLines))
						isOnHorizontalLine = true;
					if(StringUtils.isOneOf(get(cell.getWest().x, cell.getWest().y), horizontalLines))
						isOnHorizontalLine = true;

					boolean isOnVerticalLine = false;
					if(StringUtils.isOneOf(get(cell.getNorth().x, cell.getNorth().y), verticalLines))
						isOnVerticalLine = true;
					if(StringUtils.isOneOf(get(cell.getSouth().x, cell.getSouth().y), verticalLines))
						isOnVerticalLine = true;
	...
	public void removeArrowheads(){
		...
		for(int yi = 0; yi < height; yi++){
			for(int xi = 0; xi < width; xi++){
				Cell cell = new Cell(xi, yi);
				if(isArrowhead(cell)) set(cell.x, cell.y,  ' ');
			}
		}		
	}
	...
	public void removeColorCodes(){
		...
		while(cells.hasNext()){
			...
			set(cell.x, cell.y, ' ');
			cell = cell.getEast(); set(cell.x, cell.y, ' ');
			cell = cell.getEast(); set(cell.x, cell.y, ' ');
			cell = cell.getEast(); set(cell.x, cell.y, ' ');
		}
	}
	...
	public boolean isLine(Cell cell){
		return isHorizontalLine(cell.x, cell.y) || isVerticalLine(cell.x, cell.y);
	}
	...
	public boolean containsAtLeastOneDashedLine(CellSet set){
		...
		while(it.hasNext()) {
			Cell cell = (Cell) it.next();
			if(StringUtils.isOneOf(get(cell.x, cell.y), dashedLines)) return true;
		}
		return false;
	}
	...
	public boolean isSouthArrowhead(Cell cell){
		return (get(cell.x, cell.y) == 'v' || get(cell.x, cell.y) == 'V')
				&& isVerticalLine(cell.getNorth().x, cell.getNorth().y);
        }
	...
	public boolean isBullet(Cell cell){
		...
		if((c == 'o' || c == '*')
			&& isBlank(cell.getEast())
			&& isBlank(cell.getWest())
			&& Character.isLetterOrDigit(get(cell.getEast().getEast().x, cell.getEast().getEast().y)) )
			return true;
		return false;
	}
	...
	public void replaceBullets(){
		...
		for(int yi = 0; yi < height; yi++){
			for(int xi = 0; xi < width; xi++){
				...
				if(isBullet(cell)){
					set(cell.x, cell.y, ' ');
					set(cell.getEast().x, cell.getEast().y, '\u2022');
				}
			}
		}
	}
	...
	public void copyCellsTo(CellSet cells, TextGrid grid){
		...
		while(it.hasNext()){
			Cell cell = (Cell) it.next();
			grid.set(cell.x, cell.y, this.get(cell.x, cell.y));
		}
	}
	...
	private CellSet seedFill(Cell seed, char newChar){
		...
		char oldChar = get(seed.x, seed.y);
		...
		while(!stack.isEmpty()){
			...
			if(get(nCell.x, nCell.y) == oldChar && !cellsFilled.contains(nCell)) stack.push(nCell);
			if(get(sCell.x, sCell.y) == oldChar && !cellsFilled.contains(sCell)) stack.push(sCell);
			if(get(eCell.x, eCell.y) == oldChar && !cellsFilled.contains(eCell)) stack.push(eCell);
			if(get(wCell.x, wCell.y) == oldChar && !cellsFilled.contains(wCell)) stack.push(wCell);
		}
	...
	private CellSet seedFillOld(Cell seed, char newChar){
		...
		while(!stack.isEmpty()){
			...
			if(get(nCell.x, nCell.y) == oldChar) stack.push(nCell);
			if(get(sCell.x, sCell.y) == oldChar) stack.push(sCell);
			if(get(eCell.x, eCell.y) == oldChar) stack.push(eCell);
			if(get(wCell.x, wCell.y) == oldChar) stack.push(wCell);
		}
	...
	public CellSet findBoundariesExpandingFrom(Cell seed){
		...
		while(!stack.isEmpty()){
			...
			if(get(nCell.x, nCell.y) == oldChar) stack.push(nCell);
			else if(get(nCell.x, nCell.y) == '*') boundaries.add(nCell);
			
			if(get(sCell.x, sCell.y) == oldChar) stack.push(sCell);
			else if(get(sCell.x, sCell.y) == '*') boundaries.add(sCell);
			
			if(get(eCell.x, eCell.y) == oldChar) stack.push(eCell);
			else if(get(eCell.x, eCell.y) == '*') boundaries.add(eCell);
			
			if(get(wCell.x, wCell.y) == oldChar) stack.push(wCell);
			else if(get(wCell.x, wCell.y) == '*') boundaries.add(wCell);
		}
